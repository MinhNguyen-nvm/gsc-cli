...
import type { OAuthClientSecrets } from '../../types/index.js';
 ...
 */
function createOAuth2Client(secrets: OAuthClientSecrets, redirectUri?: string): OAuth2Client {
 ...
 */
async function startCallbackServer(): Promise<{ port: number; server: http.Server; waitForCode: () => Promise<string> }> {
 ...
 */
export async function authenticate(customSecretsPath?: string): Promise<OAuth2Client> {
...
export async function loadSavedCredentials(): Promise<OAuth2Client | null> {
  ...
  let secrets: OAuthClientSecrets;
  try {
  ...
  if (isExpired && tokenData.refresh_token) {
    try {
      ...
      throw new Error(
        'Your session has expired and could not be refreshed. Please re-authenticate:\n\n' +
        '  gsc auth login'
      );
    }
  }
 ...
 */
export async function getTokenInfo(): Promise<{ email: string | null; expiry: Date | null } | null> {
  const tokenData = await loadToken();
