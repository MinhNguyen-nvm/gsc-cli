...
const DEFAULT_CONFIG: CLIConfig = {
  ...
  flags: {
    ...
    dataState: (process.env.GSC_DATA_STATE as 'all' | 'final') || 'all',
  },
};
 ...
 */
export async function loadClientSecrets(customPath?: string): Promise<OAuthClientSecrets> {
  // Priority order: customPath > env var > config file > default
  let secretsPath: string | undefined = customPath;
  ...
  try {
    ...
    return JSON.parse(content) as OAuthClientSecrets;
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new Error(
        `OAuth client secrets file not found: ${secretsPath}\n\n` +
        `Please either:\n` +
 ...
 */
export async function saveSecretsPath(secretsPath: string): Promise<void> {
  const config = await loadConfig();
 ...
 */
export async function saveToken(tokenData: unknown): Promise<void> {
  await ensureConfigDir();
