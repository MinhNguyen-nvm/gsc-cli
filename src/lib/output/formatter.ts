 ...
 */
export function formatSitesTable(
 ...
 */
export function formatSitemapsTable(
  sitemaps: Array<{
    path: string;
    type: string;
    urlCount?: number;
  ...
  switch (format) {
    ...
    case 'csv':
      if (Array.isArray(data) && options?.dimensions) {
        return formatCSV(data as AnalyticsRowData[], options.dimensions);
      }
      // Fall back to JSON for non-analytics data
      return formatJSON(data);
    case 'table':
      if (Array.isArray(data) && options?.dimensions) {
        return formatTable(data as AnalyticsRowData[], options.dimensions);
      }
